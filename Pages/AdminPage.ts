...
export class AdminPage {
  ...
  async searchUser(username: string) {
    ...
    assert(await this.searchUsername.isVisible(), 'Search username field is not visible');
    await this.actions.fill(this.searchUsername, username);
    await this.actions.click(this.searchBtn);
  }

  async negativeSearchUser(username: string) {
   
    assert(!(await this.searchUsername.isVisible(), 'Search username field is visible'));
